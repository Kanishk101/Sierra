...
/// Fleet Manager geofence list — view, toggle active/inactive, create, delete.
struct GeofenceListView: View {
    ...
    var body: some View {
        ...
        .confirmationDialog(
            "Delete Geofence?",
            isPresented: .init(
                get: { vm.deleteConfirmationTarget != nil },
                set: { if !$0 { vm.deleteConfirmationTarget = nil } }
            ),
            ...
            Button("Cancel", role: .cancel) {
                vm.deleteConfirmationTarget = nil
            }
        } message: {
            Text("This will permanently remove the geofence and all its event history.")
        }
    ...
    private func geofenceRow(_ geofence: Geofence) -> some View {
        HStack(spacing: 14) {
            ...
            ZStack {
                Circle()
                    .fill(geofence.isActive ? SierraTheme.Colors.alpineMint.opacity(0.15) : Color.gray.opacity(0.1))
                    .frame(width: 40, height: 40)
                Image(systemName: geofenceIcon(geofence.geofenceType))
                    .font(.system(size: 16, weight: .semibold))
                    .foregroundStyle(geofence.isActive ? SierraTheme.Colors.alpineMint : .gray)
            }
            ...
            VStack(alignment: .leading, spacing: 3) {
                ...
                HStack(spacing: 6) {
                    Text(geofence.geofenceType.rawValue)
                    Text("·")
                    Text(formatRadius(geofence.radiusMeters))
                }
                .font(.caption)
                .foregroundStyle(.secondary)
            }
            ...
            .labelsHidden()
            .tint(SierraTheme.Colors.alpineMint)
        }
        .padding(.vertical, 4)
    }
    ...
    private func geofenceIcon(_ type: GeofenceType) -> String {
        ...
        case .restrictedZone: return "exclamationmark.shield.fill"

...
-- Fix: use role::text directly instead of get_my_role_for() which doesn't exist
...
BEGIN
    ...
    v_days_until := GREATEST(0, EXTRACT(DAY FROM (NEW.expiry_date - NOW()))::INT);
    ...
    -- role::text comparison works on the enum column directly
    FOR v_admin_id IN
        SELECT id FROM public.staff_members WHERE role::text = 'fleetManager'
    LOOP
        IF NOT EXISTS (

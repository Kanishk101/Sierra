-- ============================================================
-- Hotfix: Eliminate RLS infinite recursion via staff role helper
-- Date: 2026-03-27
--
-- Symptom:
--   PostgREST 42P17 "infinite recursion detected in policy for relation staff_members"
--
-- Root cause:
--   Multiple policies evaluate role by querying public.staff_members directly.
--   staff_members policies also query public.staff_members, creating recursive policy evaluation.
--
-- Fix:
--   1) Add SECURITY DEFINER helper `current_staff_role()` to read caller role safely.
--   2) Rewrite affected policies to use helper instead of inline staff_members subqueries.
-- ============================================================
